...
# Make the project root importable from sandbox/
sys.path.insert(0, str(Path(__file__).parent.parent))
from src.planit.config import (  # noqa: E402
    PHYSRISK_DAMAGE_HAZARDS,
    PHYSRISK_DISRUPTION_HAZARDS,
    PHYSRISK_ZEROED_HAZARDS,
)
...
SAMCHEOK_ASSET = {
    ...
    "latitude": 37.4061,   # site centroid (seawater once-through, coastal)
    "longitude": 129.1700,
    "location": "Asia",
    "capacity": 2100.0,
}
...
def _channel_label(hazard: str) -> str:
    """Return CRP channel label for a physrisk hazard type."""
    if hazard in PHYSRISK_ZEROED_HAZARDS:
        tag = "grid artefact" if hazard == "RiverineInundation" else "no KR regulation"
        return f"zeroed [{tag}]"
    if hazard in PHYSRISK_DISRUPTION_HAZARDS:
        return "disruption"
    if hazard in PHYSRISK_DAMAGE_HAZARDS:
        return "damage"
    return "unclassified"
...
def print_summary(response: dict) -> None:
    ...
    col = "{:<30} {:<12} {:<8} {:<14} {}"
    print(col.format("hazard_type", "scenario", "year", "impact_mean", "channel"))
    print("-" * 90)
    for impact in impacts:
        ...
        # Apply hazard policy: zero out flagged hazards
        if hazard in PHYSRISK_ZEROED_HAZARDS:
            mean = 0.0

        mean_str = f"{mean:.6f}" if isinstance(mean, (int, float)) else str(mean)
        channel = _channel_label(hazard)
        print(col.format(hazard, scenario, str(year), mean_str, channel))
