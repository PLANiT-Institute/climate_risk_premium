...
1. A plant-level climate-finance framework integrating transition pathways, physical adjustments, and credit-linked financing.
2. A strict reproducibility governance stack (scenario contract, frozen outputs, manifest hashes, claim registry, manuscript validator).
3. Deterministic robustness and placebo tests showing that the main transition-dominance finding is not driven by one fragile parameter setting.

This manuscript is original, is not under consideration elsewhere, and all required transparency statements are included (CRediT, funding, competing interest, data/code availability).
