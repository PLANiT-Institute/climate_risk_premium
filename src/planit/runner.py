...
class PLANiTRunner:
    ...
    def _ensure_local_venv_site_packages(self) -> None:
        ...
        Searches both .venv and .venv-climada (Python 3.12 with CLIMADA installed).
        Also scans for any Python version in each venv, not just the running one.
        """
        for venv_name in (".venv-climada", ".venv"):
            venv = Path(self._base_dir) / venv_name
            if not venv.exists():
                continue
            # Try the running Python version first, then scan for any available version
            lib_dir = venv / "lib"
    ...
    def _run_planit_subprocess(
        ...
        Falls back to subprocess execution when in-process import fails due to
        Python version mismatch (e.g., main process is 3.14 but CLIMADA needs 3.12).
        """
        import subprocess
        import json as _json
        import tempfile

        # Find .venv-climada Python (use absolute path to avoid symlink issues)
        ...
        # Build subprocess script — runs in .venv-climada Python (3.12)
        # with CLIMADA/PhysRisk natively available
...
# Override scenarios from caller
...
# Limit probabilistic seasons for speed (env override)
import os as _os
