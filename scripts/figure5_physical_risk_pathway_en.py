"""
Figure 5 (v5-EN): Physical Climate Risk — Power Plant Impact Pathway (English version)
Output spec: 1400×750 px, 150 dpi, PNG, white background
"""
...
# ─── Canvas: 1400×750 px @ 150 dpi ───
DPI = 150
FIG_W_PX, FIG_H_PX = 1400, 750
...
# ─── Geometry: wider boxes to accommodate English text ───
# Scale: 1400 px / 13 units = 107.7 px/unit
# W_H=2.80 → 302 px  W_P=4.20 → 452 px  W_F=3.00 → 323 px
W_H, W_P, W_F = 2.80, 4.20, 3.00
X_H = 0.40
X_P = X_H + W_H + 0.60   # 3.80
X_F = X_P + W_P + 0.60   # 8.60

X_HC = X_H + W_H / 2     # 1.80
X_PC = X_P + W_P / 2     # 5.90
X_FC = X_F + W_F / 2     # 10.10
...
# ═══ TITLE ═══
ax.text(6.5, 8.55, "Physical Climate Risk: Power Plant Impact Pathway",
...
for cx, txt, c in [
    (X_HC, "Hazard",           C_H_HDR),
    (X_PC, "Pathway",          C_P_HDR),
    (X_FC, "Financial Impact", C_F_HDR),
]:
...
# ═══ PATHWAY BOXES (3) ═══
for row, txt in [
    (2, "Asset Repair · Replacement"),
    (1, "Outage · Generation Loss"),
    (0, "Efficiency Loss · Output Reduction"),
]:
...
# ═══ FINANCIAL BOXES (2) ═══
for y_bot, txt in [
    (F_CAP_BOT, "Increased CapEx"),
    (F_REV_BOT, "Reduced Operating Profit"),
]:
...
arr(X_HR, Y[0] + ITEM_H * HI,  X_P, YM[1])
arr(X_HR, Y[0] + ITEM_H * LO,  X_P, YM[0])
...
# ═══ SAVE ═══
out = Path(__file__).parent.parent / "results" / "figures" / "figure5_physical_risk_pathway_en.png"
out.parent.mkdir(parents=True, exist_ok=True)
plt.savefig(out, dpi=DPI, facecolor="white")
plt.close()
print(f"Saved: {out}  ({FIG_W_PX}×{FIG_H_PX} px @ {DPI} dpi)")
