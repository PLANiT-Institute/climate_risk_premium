...
ax.text(baseline_npv - 35, 2.58, '기준 시나리오',
        ha='right', va='bottom', fontsize=9, color='#595959', style='italic')

# 순현재가치 labels inside bars, near the outer (left) tip
for i, val in enumerate(values):
    ax.text(val + 45, i, f'-{abs(val):,.0f}M',
            ha='left', va='center', fontsize=10, color='white', fontweight='bold')

# Y-axis: English scenario names
ax.set_yticks(range(len(labels)))
ax.set_yticklabels(labels, fontsize=12)

# X-axis
ax.set_xlim(-3200, 0)
ax.set_xlabel('순현재가치 (백만 달러)', fontsize=11, labelpad=8)
ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'{int(x):,}'))
ax.tick_params(axis='x', labelsize=9.5)

# Title
ax.set_title('시나리오별 순현재가치 비교',
...
# Footnote
fig.text(0.01, 0.012, '출처: PLANiT CRP 모델 results/scenario_comparison.csv',
         ha='left', va='bottom', fontsize=8, color='gray', style='italic')

plt.tight_layout(rect=[0, 0.03, 1, 1])

output_path = Path(__file__).parent.parent / 'results' / 'figures' / 'figure4_npv_comparison.png'
